...
public class accountManagement {
    ...
    public static boolean clickOnMostRecentIdentity() {
        try {
            MobileElement lastIdentity = scrollToLastIdentity();
            if (lastIdentity == null) {
                log.error("No identities found in the Identity RecyclerView.");
                return false;
            }
    ...
    public static boolean verifyMostRecentIdentityName(String expectedName, int timeoutInSeconds) {
        try {
            ...
            if (expectedName.equals(actualName)) {
                ...
                log.error("Last identity name verification FAILED. Actual: " + actualName + ", Expected: " + expectedName);
                return false;
            }
    ...
    private static MobileElement scrollToLastIdentity() {
        ...
        int maxScrolls = 30;
        int scrollCount = 0;
        MobileElement lastIdentity = null;

        while (scrollCount < maxScrolls) {
            List<MobileElement> identities = driver.findElements(identityLocator);
            if (!identities.isEmpty()) {
                lastIdentity = identities.get(identities.size() - 1);
                if (lastIdentity.isDisplayed()) {
                    break;
                }
            }
