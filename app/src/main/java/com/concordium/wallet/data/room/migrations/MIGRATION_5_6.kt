...
val MIGRATION_5_6 = object : Migration(5, 6) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE `identity_table` ADD COLUMN `identity_provider_id` INTEGER NOT NULL DEFAULT 0")
        db.execSQL("ALTER TABLE `identity_table` ADD COLUMN `identity_index` INTEGER NOT NULL DEFAULT 0")
        ...
        db.query("SELECT `id`, `identity_provider` FROM `identity_table`")
        ...
        db.execSQL("CREATE TABLE IF NOT EXISTS `account_contract_table` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `account_address` TEXT NOT NULL, `contract_index` TEXT NOT NULL)")
        db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS `index_account_contract_table_account_address_contract_index` ON `account_contract_table` (`account_address`, `contract_index`)")
        db.execSQL("CREATE TABLE IF NOT EXISTS `contract_token_table` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `contract_index` TEXT NOT NULL, `token_id` TEXT NOT NULL, `account_address` TEXT, `is_fungible` INTEGER NOT NULL, `token_metadata` TEXT, `contract_name` TEXT NOT NULL DEFAULT '')")
        ...
        db.execSQL("ALTER TABLE `_new_account_table` RENAME TO `account_table`")

        db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS `index_account_table_address` ON `account_table` (`address`)")
        db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS `index_identity_table_identity_provider_id_identity_index` ON `identity_table` (`identity_provider_id`, `identity_index`)")
    }
}
