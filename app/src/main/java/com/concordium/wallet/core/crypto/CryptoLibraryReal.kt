...
import com.concordium.mobile_wallet_lib.create_credential
...
import com.concordium.mobile_wallet_lib.create_id_request_and_private_data
...
import com.concordium.mobile_wallet_lib.generate_accounts
...
import com.concordium.wallet.data.cryptolib.IdRequestAndPrivateDataInput
...
class CryptoLibraryReal(val gson: Gson) : CryptoLibrary {
    ...
    override suspend fun createCredential(credentialInput: CreateCredentialInput): CreateCredentialOutput? =
        withContext(Dispatchers.Default) {
            ...
            Log.d("Output (Code ${result.result}): ${result.output}")
            if (result.result == CryptoLibrary.SUCCESS) {
                return@withContext gson.fromJson(result.output, CreateCredentialOutput::class.java)
            }
            Log.e("Cryptolib failed")
            return@withContext null
        }

    override suspend fun createCredentialV1(credentialInput: CreateCredentialInputV1): CreateCredentialOutput? =
        withContext(Dispatchers.Default) {
            val input = gson.toJson(credentialInput)
            loadWalletLib()
